    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['placeOrder'])) {
        if (empty($_SESSION['cart'])) {
            ...
            if (!$insufficientStock) {
                ...
                PersonalNumber = '$personalNumber' AND Phone = '$phone' AND
                Address = '$address' AND PostalCode = '$postalCode' AND 
                City = '$city' AND Email = '$email'";

                $result = $connection->query($checkCustomerSql);
                ...
                if ($result->num_rows > 0) {
                    ...
                    VALUES ('$firstName', '$lastName', '$personalNumber', '$phone', '$address', '$postalCode', '$city', '$email')";

                    $connection->query($addCustomerSql);

                    $customerID = $connection->insert_id;
                }

                $createOrderSql = "INSERT INTO Orders (CustomerID, Status, OrderDate, TotalAmount)
                VALUES ('$customerID', 'Processing', NOW(), 0.00)";

                $connection->query($createOrderSql);
